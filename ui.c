...
static inline void calc_needle_pos(float val, const float xoff, float * const x, float * const y) {
	const float _xc = 149.5 + xoff;
	const float _yc = 209.5;

	if (val < 0.00f) val = 0.00f;
	if (val > 1.05f) val = 1.05f;
	val = (val - 0.5f) * 1.5708f;

	*x = _xc + sinf (val) * 180.0f;
	*y = _yc - cosf (val) * 180.0f;
}
...
static void draw_needle (MetersLV2UI* ui, cairo_t* cr, float val,
	...
	calc_needle_pos(val, xoff, &px, &py);

	cairo_new_path (cr);
	cairo_move_to (cr, _xc, _yc);
...
static gboolean expose_event(GtkWidget *w, GdkEventExpose *ev, gpointer handle) {
	...
	if (ui->type == MT_COR) {
		...
		for (c=0; c < ui->chn; ++c) {
			...
			draw_needle (ui, cr, ui->lvl[c], 300.0 * c, col, 1.4);
		}
	}
...
static void invalidate_area(MetersLV2UI* ui, int c, float old, float new) {
	...
	calc_needle_pos(old, xoff, &ox, &oy);
	calc_needle_pos(new, xoff, &nx, &ny);

	GdkRectangle update_rect;
	update_rect.x = MAX2(0, MIN3(_xc, ox, nx) - 1);
	update_rect.y = MAX2(0, MIN3(_yc, oy, ny) - 1);
	update_rect.width = 2 + MAX3(_xc, ox, nx) - update_rect.x;
	update_rect.height = 136 - update_rect.y;
